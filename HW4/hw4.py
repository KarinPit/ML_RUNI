...
def cross_validation(X, y, folds, algo, random_state):
    ...
    for i in range(folds):
        val_idx = indices[i * fold_size : (i + 1) * fold_size] # current fold (validation)
...
class EM(object):
    ...
    def fit(self, data):
        ...
        for i in range(self.n_iter):
            ...
            for j in range(self.k):
                weighted_pdfs[:, j] = self.weights[j] * norm_pdf(data, self.mus[j], self.sigmas[j]).flatten()
...
class NaiveBayesGaussian(object):
    ...
    def fit(self, X, y):
        ...
        for class_value in classes:
            ...
            prior = len(dataset_per_class) / len(dataset) # Calculate prior probability of the class
